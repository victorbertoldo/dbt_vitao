-- Compile + execution test: flatten_json in columns mode with schema_override.
-- Validates nested object extraction, null handling, and alias normalization.
-- Run: dbt run --models test_flatten_json_columns

{{ dbt_vitao.flatten_json(
    relation=ref('json_edge_cases'),
    json_column='payload',
    mode='columns',
    schema_override=[
        {'path': 'id',                        'cast_type': 'integer', 'alias': 'payload_id'},
        {'path': 'customer.name',             'cast_type': 'text',    'alias': 'customer_name'},
        {'path': 'customer.email',            'cast_type': 'text',    'alias': 'customer_email'},
        {'path': 'customer.address.city',     'cast_type': 'text',    'alias': 'customer_city'},
        {'path': 'customer.address.country',  'cast_type': 'text',    'alias': 'customer_country'},
        {'path': 'external_code',             'cast_type': 'text',    'alias': 'external_code'},
        {'path': 'url',                       'cast_type': 'text',    'alias': 'url'},
        {'path': 'flags.active',              'cast_type': 'boolean', 'alias': 'flag_active'},
        {'path': 'flags.vip',                 'cast_type': 'boolean', 'alias': 'flag_vip'}
    ],
    prefix='',
    include_source_columns=true
) }}
